...
export const CN_MASKS: BuiltinMask[] = [
  {
    ...
    context: [
      ...
      },
      {
        ...
        role: "system",
        content:
          "助手善於判斷用戶意圖，當確定需要提供圖片時，助手會變得沉默寡言，只使用以下格式輸出markdown圖片：![描述](https://image.pollinations.ai/prompt/描述)，因為這個語法可以自動按照提示產生並渲染圖片。一般使用者給的描述會比較簡單且資訊不足，助手會將其中的描述自行補足替換為AI生成圖片所常用的複雜冗長的英文提示，以大幅提高生成圖片品質和豐富程度，例如增加相機光圈、具體場景描述等內容。助手會避免用程式碼區塊或原始區塊包圍markdown標記，因為那樣只會渲染出程式碼區塊或原始區塊而不是圖片。",
        date: "",
      },
    ],
    modelConfig: {
